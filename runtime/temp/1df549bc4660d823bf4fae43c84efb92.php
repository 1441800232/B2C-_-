<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:66:"D:\wamp64\www\shop\public/../application/index\view\flow\flow1.htm";i:1695179051;s:69:"D:\wamp64\www\shop\public/../application/index\view\common\footer.htm";i:1694164482;}*/ ?>
<!doctype html>
<html>
<head>
...
<script type="text/javascript">
    ...
    var cart_list ="<?php echo url('index/Flow/flow1'); ?>";//删除转跳
    var updata_cart ="<?php echo url('index/Flow/updateCart'); ?>";//修改购车中的商品数量地址
</script>
...
<div class="column c-action">操作</div>
    ...
    <div class="item-list" ectype="itemList">
        <?php if(is_array($cartGoodsRes) || $cartGoodsRes instanceof \think\Collection || $cartGoodsRes instanceof \think\Paginator): $i = 0; $__LIST__ = $cartGoodsRes;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$goods): $mod = ($i % 2 );++$i;?>
    <!-- 循环 -->
...
<div class="cart-checkbox" ectype="ckList">
    <input type="checkbox" id="checkItem_<?php echo $goods['goods_id_attr_id']; ?>" value="<?php echo $goods['goods_id_attr_id']; ?>" name="checkItem" class="ui-checkbox" ectype="ckGoods">
    <label for="checkItem_<?php echo $goods['goods_id_attr_id']; ?>" class="ui-label-14">&nbsp;</label>
</div>
                    </div>
                    <div class="cell s-goods">
<div class="goods-item">
    <div class="p-img">
                <a href="#" target="_blank"><img src="__UPADMIN__/<?php echo $goods['mid_thumb']; ?>" width="70" height="70"></a>
            </div>
    <div class="item-msg">
    	        <a href="#" target="_blank" ><?php echo $goods['goods_name']; ?></a>
                <div class="gds-types">
        </div>
    </div>
</div>
                    ...
                    <div class="cell s-props">
                    	<?php echo $goods['goods_attr_str']; ?>
                    </div>
                    <div class="cell s-price">
<strong id="goods_price_8"><em>¥</em><?php echo $goods['member_price']; ?></strong>
                    </div>
                    <div class="cell s-quantity">
<div class="amount-warp">
            <input type="text" value="<?php echo $goods['goods_num']; ?>" name="goods_number[8]" id="goods_number_8" onchange="change_goods_number('<?php echo $goods['goods_id_attr_id']; ?>', this.value, 2, 16)" class="text buy-num" ectype="number" defaultnumber="1">
        <div class="a-btn">
    	        <a href="#" onclick="changenum('<?php echo $goods['goods_id_attr_id']; ?>', 1, 2, 16,17,this)" class="btn-add"><i class="iconfont icon-up"></i></a>
        <a href="#" onclick="changenum('<?php echo $goods['goods_id_attr_id']; ?>', 1, 2, 16,17,this)"
           <?php if($goods['goods_num']): ?>  class="btn-reduce btn-disabled"
            <?php else: ?>  class="btn-reduce" <?php endif; ?>
           class="btn-reduce btn-disabled"><i class="iconfont icon-down"></i></a>
            </div>
    </div>
<div class="tc ftx-03">有货</div>
                    </div>
                    <div class="cell s-sum">
<strong id="goods_subtotal_8"><font id="_8_subtotal"><em>¥</em><?php echo $goods['subtotal']; ?></font></strong>
...
<a href="#" id="remove_8" ectype="cartOperation" data-value="{&quot;divId&quot;:&quot;cart_remove&quot;,&quot;url&quot;:&quot;<?php echo url("Flow/dropGoods",array('id_attr'=>$goods['goods_id_attr_id']),''); ?>&quot;,&quot;cancelUrl&quot;:&quot;flow.php?step=drop_to_collect&amp;id=8&quot;,&quot;recid&quot;:8,&quot;title&quot;:&quot;删除&quot;}" class="cart-remove">删除</a>
<a href="#" id="store_8" ectype="cartOperation" data-value="{&quot;divId&quot;:&quot;cart_collect&quot;,&quot;url&quot;:&quot;flow.php?step=drop_to_collect&amp;id=8&quot;,&quot;recid&quot;:8,&quot;title&quot;:&quot;关注&quot;}" class="cart-store">收藏</a>
    ...
    <div class="cart-checkbox cart-all-checkbox" ectype="ckList">
	    ...
	    <div class="operation">
        <a href="#" class="cart-remove-batch" data-dialog="remove_collect_dialog" data-divid="cart-remove-batch" data-removeurl="<?php echo urL('Flow/deleteCartGoods'); ?>" data-collecturl="ajax_dialog.php?act=drop_to_collect" data-title="删除">删除选中的商品</a>
        ...
        <div class="comm-right">
            <div class="btn-area">
                <form name="formCart" method="post" action="<?php echo url('index/Flow/flow2'); ?>" onsubmit="return get_toCart();">
                    <input name="goPay" type="submit" class="submit-btn" value="去支付" <?php if(!isset($_SESSION['think']['username'])){echo 'id="go_pay"'; }?>    data-url="<?php echo url('index/Flow/flow1','',''); ?>"/>
...
<script type="text/javascript" src="__index__/js/warehouse.js"></script><script type="text/javascript" src="__index__/js/warehouse_area.js"></script>
    ...
    <script type="text/javascript">
        ...
        function change_goods_number(rec_id, goods_number, warehouse_id, area_id, cValue, favourable_id){
            ...
            Ajax.call("<?php echo url('index/Flow/updateCart'); ?>", 'rec_id=' + rec_id + '&sel_id=' + str + '&sel_flag=' + 'cart_sel_flag' +'&goods_number=' + goods_number +'&cValue=' + cValue +'&warehouse_id=' + warehouse_id +'&area_id=' + area_id +'&favourable_id=' + favourable_id, change_goods_number_response, 'POST','JSON');
        }
